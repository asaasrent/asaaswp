...
class Properties
{
    ...
    private function update_data($property_id, $property)
    {
        ...
        if (isset($property['mediaData']) && isset($property['mediaData']['vt']) && count($property['mediaData']['vt'])) {
        ...
        if ($offer_type instanceof Term) {
            wp_set_object_terms($property_id, array($offer_type->get_ID()), $offer_type->get_wp_term()->taxonomy);
        }
