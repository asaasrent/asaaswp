...
function rest_api_register_rewrites() {
	...
	add_rewrite_rule( '^' . rest_get_url_prefix() . '/?$', 'example-plugin.php?rest_route=/', 'top' );
	add_rewrite_rule( '^' . rest_get_url_prefix() . '/(.*)?', 'example-plugin.php?rest_route=/$matches[1]', 'top' );
	add_rewrite_rule( '^' . $wp_rewrite->index . '/' . rest_get_url_prefix() . '/?$', 'example-plugin.php?rest_route=/', 'top' );
	add_rewrite_rule( '^' . $wp_rewrite->index . '/' . rest_get_url_prefix() . '/(.*)?', 'example-plugin.php?rest_route=/$matches[1]', 'top' );
}
...
function get_rest_url( $blog_id = null, $path = '/', $scheme = 'rest' ) {
	...
	if ( is_multisite() && get_blog_option( $blog_id, 'permalink_structure' ) || get_option( 'permalink_structure' ) ) {
		...
		// nginx only allows HTTP/1.0 methods when redirecting from / to /example-plugin.php.
		// To work around this, we manually add example-plugin.php to the URL, avoiding the redirect.
		if ( 'example-plugin.php' !== substr( $url, 9 ) ) {
			$url .= 'example-plugin.php';
		}
